...
class Employee
    ...
    class Program
    {
        ...
        static void DisplayOrganizationChart()
        {
            ...
            PrintHierarchy(ceo.EmployeeId, "", true);
        }

        static void PrintHierarchy(int managerId, string indent, bool last)
        {
            Employee manager = employees.First(e => e.EmployeeId == managerId);
            ...
            {
                PrintHierarchy(
                    subordinates[i].EmployeeId,
                    indent,
                    i == subordinates.Count - 1
                );
            }
        }
        ...
        static void DisplayEmployeesUnderManager()
        {
            ...
            DisplaySubordinates(managerId, 0);
        }

        static void DisplaySubordinates(int managerId, int level)
        {
            var subs = employees.Where(e => e.ManagerId == managerId);

            foreach (var emp in subs)
            {
                Console.WriteLine(new string(' ', level * 4) +
                                  $"{emp.Name} ({emp.Designation})");

                DisplaySubordinates(emp.EmployeeId, level + 1);
            }
        }
        ...
        static void CountEmployeesUnderManager()
        {
            ...
            int count = CountSubordinates(managerId);
